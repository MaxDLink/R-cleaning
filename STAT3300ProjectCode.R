...
head(ev_data) #view first 6 rows of data to see variable names and data formatting
nrow(ev_data) #view number of rows
ncol(ev_data) #view number of columns
...
### Merge based on State and County 
...
#ACTUALWORK####################################################################
library(ggplot2)
#Converting the numerical binary groups into 2 broad categories
omitted_merged_data$Metro_Status_Numeric=
  ifelse(omitted_merged_data$Metro_Status_Numeric==0,"NonMetro","Metro")
head(omitted_merged_data)

#Removed the VPU_Numeric category since it is not needed
omitted_merged_data=omitted_merged_data[,c(1,2,3,4,6,7,8)]
head(omitted_merged_data)

#Renamed Metro_Status_Numeric
names(omitted_merged_data)[7]<-"MetroType"
head(omitted_merged_data)

#Code for the Interaction Plot
ggplot(omitted_merged_data,aes(x=VPU,y=log(PEV+1),group=MetroType,
  color=MetroType))+
...
#Code for the Side-by-Side Boxplot
ggplot(omitted_merged_data,aes(x=VPU,y=log(PEV+1),fill=MetroType))+
  geom_boxplot()+
  ggtitle("Boxplot")

#Summary stats
library(tidyverse)
data.summary=omitted_merged_data %>% group_by(VPU,MetroType) %>%
  summarise(Count=n(),Mean=mean(log(PEV+1)),StDev=sd(log(PEV+1))) %>% as.data.frame()
data.summary


#Anova Test
model=aov(log(PEV+1)~VPU*MetroType,omitted_merged_data)
summary(model)
